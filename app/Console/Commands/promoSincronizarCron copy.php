        ...
        foreach($direccionesIP as $serverStore) { 
            ...
            if($conexionStore == true) {
                ...
                try {
                    ...
                    $querynumHQ = sqlsrv_query($conexionStore, "select count(dpHQ.itemlookupcode) numHQ from opendatasource('sqloledb', 'server=192.168.222.4; user id=sa;password=sa2000').fchqdb.dbo.L@Red_Software_DescuentosGlobales_Articulos_Promocional dpHQ where dpHQ.storeid = (select storeid from configuration)");
                    $querynumStore = sqlsrv_query($conexionStore, "select count(itemlookupcode) numStore from L@Red_Software_DescuentosGlobales_Articulos_Promocional");

                    $numHQ;
                    $numStore;

                    //Validar que no hubo error en los queries, caso contrario saltar sincronizacion
                    if($querynumHQ != false && $querynumStore != false) {

                        while ($rows = sqlsrv_fetch_array($querynumHQ, SQLSRV_FETCH_ASSOC)) {
                            $numHQ = $rows['numHQ'];
                        }
    
                        while ($rows = sqlsrv_fetch_array($querynumStore, SQLSRV_FETCH_ASSOC)) {
                            $numStore = $rows['numStore'];
                        }
